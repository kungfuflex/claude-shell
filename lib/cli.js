...
async function main() {
    ...
    commander_1.program
        ...
        .option("-m, --model <model>", "Model to use (default: claude-3-5-sonnet-20241022)")
        ...
        .option("-b, --blessed", "Use blessed UI interface instead of readline", false)
        .parse(process.argv);
    const options = commander_1.program.opts();
    // Initialize Shell
    const shell = new shell_js_1.Shell({
        ...
        shell.on("message", ({ role, content }) => {
            switch (role) {
                case "user":
                    ...
                    break;
                case "assistant":
                    console.log(chalk_1.default.green("Claude:"), content);
                    break;
                case "tool":
                    const result = content;
                    if (result.output) {
                        console.log(chalk_1.default.cyan("Tool Output:"), result.output);
                    }
                    if (result.error) {
                        console.log(chalk_1.default.red("Tool Error:"), result.error);
                    }
                    if (result.base64_image) {
                        console.log(chalk_1.default.yellow("Screenshot captured"));
                    }
                    break;
            }
        });
    ...
    if (options.blessed) {
        ...
        const ui = new ui_js_1.UI({ debug: options.debug });
        // Show warning
        ...
        shell.on("error", (error) => {
            ui.addMessage("system", chalk_1.default.red("Error: " + error.message));
        });
    }
    else {
        // Use traditional readline interface
        console.log(chalk_1.default.yellow(WARNING_TEXT));
        console.log();
    ...
    if (params.action === "screenshot") {
        ...
        return new Promise((resolve) => {
            exec("DISPLAY=:1 import -window root /tmp/screenshot.png", async (error) => {
                ...
                try {
                    const imageData = await fs_1.promises.readFile("/tmp/screenshot.png");
...
async function handleEditorTool(params) {
    ...
    try {
        switch (command) {
            ...
            case "create":
                await fs_1.promises.writeFile(path, file_text);
            ...
            case "str_replace": {
                ...
                await fs_1.promises.writeFile(path, newContent);
            ...
            case "insert": {
                ...
                await fs_1.promises.writeFile(path, lines.join("\n"));
