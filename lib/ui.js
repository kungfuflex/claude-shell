...
class UI {
    ...
    appendContent(text) {
        ...
        // Find the last Claude line (working backwards)
        let lastClaudeLineIndex = -1;
        for (let i = lines.length - 1; i >= 0; i--) {
            if (lines[i].includes(this.formatter.format('{blue.bold}Claude{/}'))) {
                lastClaudeLineIndex = i;
                break;
            }
        }
        if (lastClaudeLineIndex !== -1) {
            // Get all lines after the Claude header, join them with the new text
            const contentLines = lines.slice(lastClaudeLineIndex + 1);
            const existingContent = contentLines.join('\n');
            const newContent = existingContent + text;
            // Get the header line
            const headerLine = lines[lastClaudeLineIndex];
            // Replace content while preserving the header
            this.chatBox.setContent(headerLine + '\n' + newContent);
